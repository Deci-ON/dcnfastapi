...
async def get_current_user(db: AsyncSession = Depends(get_session), token: str = Depends(oauth2_schema)) -> UsuarioModel:
    ...
    try:
        ...
        if user_id is None:
            logger.error("ID do usuário não encontrado no payload do token")
    ...
    try:
        ...
        logger.error("ID do usuário no token não é um número inteiro")
        raise credential_exception
    ...
    async with db as session:
        ...
        logger.info(f"Resultado da query: {usuario}")
